...
import pandas as pd
import numpy as np
from cvxpy import *  # use cvxpy version 0.4.11

# read monthly_prices.csv
mp = pd.read_csv("monthly_prices.csv",index_col=0)
# mp = pd.read_csv("monthly_prices_16-18.csv",index_col=0)
mr = pd.DataFrame()
...
for s in mp.columns:
    ...
    for t in range(1,len(mp.index)):
        ...
        # Warning: set_value is deprecated and will be removed in a future release.
...
# Warning: Method .as_matrix will be removed in a future version. Use .values instead.
...
n = len(symbols)
x = Variable(n)
req_return = 0.02  # hardcoded by looking at Microsoft's return in r
ret = r.T*x
risk = quad_form(x, C)
